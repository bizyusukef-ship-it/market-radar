...
## 過去14日間のふりかえり（アーカイブ）
`generate_dashboard.py` の `build_archive_html()` が、対象日の前日から14日分の
`events_*.json` を読んで自動生成する。
- 「主要イベント（要約対象）」= `highlights` を持つ非定型イベント（FOMC・日銀会合）。
  要約済みなら内容を展開表示、未記入なら「要約はまだ書かれていません」バッジを出す
- その下に日別リスト（日付を押すとその日の `dashboard_*.html` へ）
- イベントが無かった日は「主要イベントなし: ...」と1行にまとめる
